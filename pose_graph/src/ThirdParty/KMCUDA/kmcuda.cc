...
#pragma omp parallel for
    ...
    case kmcudaInitMethodPlusPlus:
    {
      ...
      {
        if (verbosity > 1 || (verbosity > 0 && (clusters_size < 100 || i % (clusters_size / 100) == 0)))
        {
          printf("\rstep %d", i);
          fflush(stdout);
        }
...
#pragma omp simd reduction(+ \
    ...
    case kmcudaInitMethodAFKMC2:
    {
      ...
      {
        if (verbosity > 1 || (verbosity > 0 && (clusters_size < 100 || k % (clusters_size / 100) == 0)))
        {
          printf("\rstep %d", k);
          fflush(stdout);
        }
  ...
  {
    DEBUG("arguments: %d %p %.3f %.2f %d %" PRIu32 " %" PRIu16 " %" PRIu32 " %" PRIu32 " %" PRIu32 " %d %" PRIi32 " %p %p %p %p\n", init, init_params,
...
#pragma omp parallel for
    ...
    else
    {
      ...
      FOR_EACH_DEVI(
          if (static_cast<int32_t>(devi) == origin_devi) {
            continue;
          } CUCH(cudaMemcpyPeerAsync(neighbors + std::get<0>(nplan[devi]) * k, device_ptrs,
