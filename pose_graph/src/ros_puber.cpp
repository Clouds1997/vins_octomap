...
RosPuber::RosPuber ( ros::NodeHandle& nh )
{
    ...
    // Current frame.
    // puber_robot_pose_ = nh.advertise<geometry_msgs::PoseStamped> ( "rio_slam/cur_robot_pose", 1, true );
    // image_transport::ImageTransport it ( nh );
    // puber_img_match_ = it.advertise ( "rio_slam/cur_img_match", 1, true );

    // // Dynamic pixel culling.
    // puber_dpc_img_objects_ = it.advertise ( "rio_slam/dpc_img_objects", 1, true );
    // puber_dpc_img_clusters_ = it.advertise ( "rio_slam/dpc_img_clusters", 1, true );
    // puber_dpc_img_mask_ = it.advertise ( "rio_slam/dpc_img_mask", 1, true );

    // KFs and pose graph. Sparse Map
    // puber_mappoints_ = nh.advertise<sensor_msgs::PointCloud> ( "rio_slam/mappoints", 1, true );
    // puber_kfs_puber_ = nh.advertise<geometry_msgs::PoseArray> ( "rio_slam/keyframes", 1, true );
    // puber_encoder_graph_ = nh.advertise<visualization_msgs::Marker> ( "rio_slam/encoder_graph", 1, true );
    // puber_loop_graph_ = nh.advertise<visualization_msgs::Marker> ( "rio_slam/loop_graph", 1, true );
    // puber_visual_graph_ = nh.advertise<visualization_msgs::Marker> ( "rio_slam/visual_graph", 1, true );
